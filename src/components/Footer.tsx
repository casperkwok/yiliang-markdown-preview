...
export const Footer: React.FC<FooterProps> = ({
  ...
  const handleExportPDF = async (filename: string) => {
    ...
    const result = await handleExport(filename);
    if (result.success) {
      ...
      throw new Error(result.message || t("errors.exportFailed"));
    }
  };
  ...
  const buttonBaseClass =
    "px-3 py-1.5 font-bold rounded-md transition-colors text-xs flex items-center cursor-pointer";
  ...
  return (
    <footer
      className={`fixed bottom-0 left-0 right-0 z-20 border-t bg-opacity-100 ${
        ...
        {!isEditMode ? (
          <>
            ...
            <div className="flex justify-center">
              <div
                className={`py-1.5 flex items-center justify-center text-xs ${
              ...
              >
                ...
                <span className="text-xs whitespace-nowrap">
                  {content.length} {t("footer.characters")}
                </span>
              </div>
            </div>
